...
public class AnimatedSvgView extends View {
    ...
    private static final String TAG = "AnimatedSvgView_xunxun";
    ...
    private void rebuildGlyphData() {
        SvgPathParser parser = new SvgPathParser() {
            ...
            protected float transformX(float x) {
                return x * mWidth / mViewport.x + leftOffset;
            }
...
//            Log.d("xunxun", "onDraw:phase=" + phase);
            ...
            if (startLoopTrace) {
                float alphaPhase = phase;
                if (alphaPhase > 0.5)
                    alphaPhase = 1 - alphaPhase;
                mGlyphData[i].paint.setAlpha((int) (alphaPhase * 255));
            }
            mGlyphData[i].paint.setPathEffect(new DashPathEffect(
                    new float[]{
...
//        ViewCompat.postInvalidateOnAnimation(this);
        ...
        if (!startLoopTrace && t > mFillStart) {
            ...
            if(needDownAlpha && t > mFillStart && t<(mFillStart+mFillTime)) {

                // 绘制渐变出现的过程，即改变alpha过程
                float phase = MathUtil.constrain(0, 1, (t - mFillStart) * 1f / mFillTime);
                for (int i = 0; i < mGlyphData.length; i++) {
                    GlyphData glyphData = mGlyphData[i];
                    mFillPaint.setARGB((int) ((1f-phase) * ((float) mFillAlphas[i] / (float) 255) * 255),
                            mFillReds[i],
                            mFillGreens[i],
                            mFillBlues[i]);
                    canvas.drawPath(glyphData.path, mFillPaint);
